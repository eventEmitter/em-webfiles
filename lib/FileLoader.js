										...
										if ( err ) callback( err );
											...
											if ( stats.isDirectory() ){
												...
												fs.readFile( path.join( dir, file ), function( err, data ){
													if ( !err ) tree[ file ] = new File( { data: data, filename: file, path: path.join( basepath, file ), abspath: path.join( dir, file ) } );
													cb( err );
												}.bind( this ) );
											}
										}
				...
				if ( !err ){
					if ( stats.isDirectory() ){
						...
						fs.readFile( path.join( dir, filename ), function( err, data ){
							if ( !err ) {
								tree[ filename ] = new File( { data: data, filename: filename, path: path.join( basepath, filename ), abspath: path.join( dir, filename ) } );
								this.emit( "add", relativePath, filename, tree[ filename ] );
							}
						}.bind( this ) );																								
					}
