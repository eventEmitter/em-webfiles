!function() {
	...
	module.exports = new Class({
		...
		, _handleHashRemove: function(hash, file) {
			if (debug) log.info('File «'+filename+'» was removed ...');

			async.each(this.modules, function(module, next){
				module.remove(parentPath, filename, subtree, this.tree, next);
			}.bind(this), function(){});
		}


		, _handleHashAdd: function(hash, file) {
			if (debug) log.info('File «'+filename+'» was added ...');

			async.each(this.modules, function(module, next){
				module.add(parentPath, filename, subtree, this.tree, next);
			}.bind(this), function(){});
		}
