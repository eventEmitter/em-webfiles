	...
	module.exports = new Class( {
		...
		, request: function( request, response, next ){
			if ( Object.hasOwnProperty( this.hashTree, request.pathname ) ){
				...
				if ( file ){
					if ( file.isDirectory ){
						...
						for( var i = 0, l = this.directoryIndex.length; i < l; i++ ){
							if ( Object.hasOwnProperty( file, this.directoryIndex[ i ] ) && !file[ this.directoryIndex[ i ] ].isDirectory ){
								file = file[ this.directoryIndex[ i ] ];

								// thats the file we're looking for

								// check for etag
								if ( response.getHeader( "If-None-Match" ) === file.etag ){
									response.send( "", { Etag: file.etag, "Content-Type": file.contentType }, 304 );
								}
								else {
									response.send( file[ this.directoryIndex[ i ] ].data, { Etag: file.etag, "Content-Type": file.contentType }, 200 );
								}
								return;
							}
						}
						...
						while( i-- ){
							if ( this.filters.test( file.filename ) ){
								next();
								return;
							}
						}
