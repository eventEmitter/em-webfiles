	...
	module.exports = new Class( {
		...
		, __handleChange: function(){
			if ( this.__livereload ){
				try{
					this.__livereload.send( 200, "window.location.reload();", { "Content-Type": "Application/Javascript" } );
				} catch ( e ){
		...
		, request: function( request, response, next ){
			...
			// live reload
			else if ( request.pathname === "/__em_webfiles_live_reload.js" ){
				// let this request hnag
				this.__livereload = response;
				return;
			}
